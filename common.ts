...
import { readableStreamToString, ClayHrError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new ClayHrError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new ClayHrError(e, e.response?.data)
                }
                if (innerError instanceof ClayHrError) {
                    // Got "ClayHrError" from the above try block
                    throw innerError;
                }
